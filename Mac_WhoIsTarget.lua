...
function Mac_WhoIsTargetMixin:OnEvent(event)
  ...
  if isArena then -- проверяет находиться ли игрок на арене
...
function Mac_WhoIsTargetMixin:OnUpdate()
  ...
  for i = 1, GetNumArenaOpponents() do
    if GetUnitName("arena" .. i .. "target") == GetUnitName("player") then
      if UnitIsDead("arena" .. i) ~= true then -- проверяет живой ли аппонент
        ...
        --  print("этот аппонент", GetUnitName("arena" .. i) ,"мертв и поэтому его таргет не отображается")
      end
    end
